...
TEST(FrameTest, FragmentedFrame) {
    ...
    std::vector<uint8_t> frag2 =
        b.build(opcode::CONTINUATION, reinterpret_cast<const uint8_t *>(text.data() + 10), 10,
                false, false);
    EXPECT_EQ(frag2[0] & 0x80, 0); // FIN not set
    EXPECT_EQ(frag2[0] & 0x0F, static_cast<uint8_t>(opcode::CONTINUATION));

    // Final fragment
    std::vector<uint8_t> frag3 =
        b.build(opcode::CONTINUATION, reinterpret_cast<const uint8_t *>(text.data() + 20),
                text.size() - 20, true, false);
