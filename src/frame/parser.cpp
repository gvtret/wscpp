...
parse_result parser::parse(const uint8_t* data, size_t size, frame_header& header, std::vector<uint8_t>& payload) {
    size_t consumed = 0;
    ...
    while (consumed < size) {
        ...
        switch (state_) {
            ...
            case state::PAYLOAD_LENGTH_EXT:
                if (parse_payload_length(data + consumed, size - consumed, consumed) == parse_result::ERROR) {
                    return parse_result::ERROR;
                }
                break;
                
            case state::MASKING_KEY:
                if (parse_masking_key(data + consumed, size - consumed, consumed) == parse_result::ERROR) {
                    return parse_result::ERROR;
                }
                break;
